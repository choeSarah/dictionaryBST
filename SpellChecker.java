 ...
 * 
 * @author Erin Parker and Ishita Juluru and Sarah Choe
...
public class SpellChecker {
	...
	public List<String> spellCheck(File documentFile) {
		...
		for(int i = 0 ; i< wordsToCheck.size(); i++) { //iterates through the wordsToCheck list
			String word = wordsToCheck.get(i);
			if(!(dictionary.contains(word))){ //if the word is not in the dictionary
				misspelled.add(word); //add it to the misspelled list
			}
		}
		return misspelled; //return the misspelled list
	}
